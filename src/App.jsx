...
function updateNode(tasks, id, fn) {
  return tasks.map((t) =>
    t.id === id ? fn(t) : { ...t, children: updateNode(t.children, id, fn) }
  );
}
function deleteNode(tasks, id) {
  return tasks
    .filter((t) => t.id !== id)
    .map((t) => ({ ...t, children: deleteNode(t.children, id) }));
}
function addChild(tasks, parentId, newTask) {
  return tasks.map((t) =>
    t.id === parentId
      ? { ...t, collapsed: false, children: [...t.children, newTask] }
      : { ...t, children: addChild(t.children, parentId, newTask) }
  );
}
function addSibling(tasks, siblingId, newTask) {
  for (let i = 0; i < tasks.length; i++) {
    if (tasks[i].id === siblingId) {
      const next = [...tasks];
      next.splice(i + 1, 0, newTask);
      return next;
    }
  }
...
function TaskNode({ task, depth, tasks, setTasks }) {
  ...
  const addChildTask = () => {
    const nt = makeTask("");
    setTasks((t) => addChild(t, task.id, nt));
    setTimeout(() => document.getElementById("node-" + nt.id)?.focus(), 50);
  };
  const addSiblingTask = () => {
    const nt = makeTask("");
    setTasks((t) => addSibling(t, task.id, nt));
    setTimeout(() => document.getElementById("node-" + nt.id)?.focus(), 50);
  };
  ...
  return (
    <div>
      <div
        style={{ ...s.row, paddingLeft: indentPx }}
      ...
      >
        <button
          ...
          style={{
            ...
            transform: task.collapsed ? "rotate(-90deg)" : "rotate(0deg)",
          }}
        >
          ▾
        </button>
        ...
        <button onClick={toggle} style={{ ...s.check, ...(task.done ? s.checkDone : {}) }}>
          {task.done && (
            <svg width="9" height="7" viewBox="0 0 9 7"><path d="M1 3.5L3 5.5L8 1" stroke="white" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" fill="none"/></svg>
          )}
        </button>
        ...
        <span
          ...
          onBlur={(e) =>
            setTasks((t) => updateNode(t, task.id, (n) => ({ ...n, text: e.currentTarget.textContent.trim() })))
          }
          onKeyDown={(e) => {
          ...
          style={{
            ...
            color: task.done ? "#aaa9a0" : depth === 0 ? "#18180f" : "#3a3a30",
            fontWeight: depth === 0 ? "500" : "400",
            fontSize: depth === 0 ? "14px" : depth === 1 ? "13px" : "12.5px",
          }}
        >
          {task.text}
        </span>
        ...
        <div style={{ position: "relative" }}>
          ...
          <div>
            {task.children.map((child) => (
              <TaskNode key={child.id} task={child} depth={depth + 1} tasks={tasks} setTasks={setTasks} />
            ))}
          </div>
        </div>
      )}
    ...
    <div style={s.page}>
      ...
      <div style={s.card}>
        <div style={s.header}>
          <div>
            ...
            <p style={s.sub}>
              ...
              {tot > 0 && (
                <span style={s.bar}>
                  <span style={{ ...s.barFill, width: `${(dn / tot) * 100}%` }} />
                </span>
              )}
            </p>
          </div>
          <button onClick={addRoot} style={s.addRoot} className="add-root">
            yeni konu +
          </button>
        </div>
        ...
        <div style={s.hint}>
          ...
          <span>Enter → aynı seviye</span>
          <span style={s.dot}>·</span>
          <span>Tab → alt seviye</span>
        </div>

        <div style={s.list}>
          {tasks.length === 0 && <p style={s.empty}>henüz hiçbir şey yok.</p>}
          {tasks.map((task) => (
            <TaskNode key={task.id} task={task} depth={0} tasks={tasks} setTasks={setTasks} />
          ))}
        </div>
      </div>
    </div>
...
const s = {
  ...
  actionBtn: { background: "none", border: "none", color: "#b0ada4", fontSize: "14px", cursor: "pointer", padding: "2px 5px", borderRadius: "2px", fontFamily: "inherit", lineHeight: 1, transition: "color 0.1s, background 0.1s" },
